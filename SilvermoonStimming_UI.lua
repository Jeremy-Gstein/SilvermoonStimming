...
function UI.Refresh(db)
    ...
    if db.bestLapSeconds then
        W.best:SetText(string.format(L["LABEL_BEST"], db.bestLapSeconds))
    else
        W.best:SetText(L["LABEL_BEST_NONE"])
    end
end
...
function UI.OnStateChange(newState)
    ...
    if newState == "OFF_TRACK" then
        if W.dir then W.dir:SetText(L["STATE_OFF_TRACK"]) ; W.dir:SetTextColor(unpack(COL.dim)) end
    ...
    elseif newState == "IN_CENTER" then
        if W.dir then W.dir:SetText(L["STATE_IN_CENTER"])  ; W.dir:SetTextColor(unpack(COL.orange)) end
...
function UI.OnTick(accumulatedAngle, direction)
    ...
    if direction == CFG.DIR_CW then
        if W.dir then W.dir:SetText(L["STATE_CW"])   ; W.dir:SetTextColor(unpack(COL.blue))   end
    ...
    elseif direction == CFG.DIR_CCW then
        if W.dir then W.dir:SetText(L["STATE_CCW"]) ; W.dir:SetTextColor(unpack(COL.orange)) end
        ...
        if W.dir then W.dir:SetText(L["STATE_STARTING"])   ; W.dir:SetTextColor(unpack(COL.dim))    end
